...
const RC_STATES_ALL = ['New','Scheduled','Delivered','Cancelled'];
...
function buildRROrdersFull() {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    const hasResume = state !== 'New' || r() < 0.55;
...
function buildRCOrdersFull() {
  const r = _rand(91);
  const writers = ['—','Aditi K.','Vivek M.','Sana R.'];
  const roles = ['Senior Product Manager','Software Engineer (Backend)','Data Scientist','UX Designer','Frontend Engineer','Engineering Manager'];
  const expLevels = ['Mid-level (3–5 yrs)','Senior (6–9 yrs)','Lead / Principal (10+ yrs)'];
  const list = [];
  for (let i = 0; i < 14; i++) {
    const cand = window.CANDIDATES[Math.floor(r() * window.CANDIDATES.length)];
    const state = RC_STATES_ALL[Math.floor(r() * RC_STATES_ALL.length)];
    const placed = window.relDate(Math.floor(r() * 14), 9 + Math.floor(r()*8), Math.floor(r()*60));
    const writer = ['New','Cancelled'].includes(state) ? '—' : writers[1 + Math.floor(r()*3)];
    // Candidate post-order info — some filled, some missing (New orders often incomplete)
    const infoFilled = state !== 'New' || r() < 0.5;
    const targetRole = infoFilled ? roles[Math.floor(r() * roles.length)] : null;
    const experienceLevel = infoFilled ? expLevels[Math.floor(r() * expLevels.length)] : null;
    const hasResume = infoFilled;
    const resume = hasResume ? { name: `${cand.name.split(' ')[0]}_Resume.pdf`, uploadedAt: placed } : null;
    const jobLinks = infoFilled && r() > 0.4 ? 'https://linkedin.com/jobs/view/123456\nhttps://internshala.com/job/456789' : null;
    const availability = infoFilled && r() > 0.5 ? 'Weekdays 6–8 PM IST, Weekends anytime' : null;
    const hasReport = state === 'Delivered';
    const callReport = hasReport ? { name: 'PostCall_Report.pdf', uploadedAt: window.relDate(Math.floor(r()*3), 14, 0) } : null;
    list.push({
      id: 'RC-' + (3200 + i),
      candidate: cand, state, writer, placed,
      amountPaid: 1499,
      score: cand.score,
      targetRole, experienceLevel, resume, jobLinks, availability, callReport,
    });
  }
...
function buildMRROrders() {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    const state = MRR_STATES_ALL[Math.floor(r() * MRR_STATES_ALL.length)];
    ...
    const hasOriginalResume = state !== 'New' || r() < 0.55;
    list.push({
      id: 'MRR-' + (6100 + i),
      candidate: cand,
      plan,
...
window.rcStateTone = (s) => ({
  'New':'grey','Scheduled':'blue','Delivered':'green','Cancelled':'red'
}[s] || 'grey');
