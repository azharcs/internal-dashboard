...
function MRRListScreen({ openOrder }) {
  ...
  let filtered = orders;
  if (stateFilter.length) filtered = filtered.filter(o => stateFilter.includes(o.state));
  if (planFilter.length) filtered = filtered.filter(o => planFilter.includes(o.plan.key));
  ...
  return (
    <div className="page">
      <ServicePageHead code="MRR" title="Manual Resume Rewrite" sub="Manual delivery · Resume Pro ₹2,300 · Resume Package ₹2,800"
        actions={<>
          <button className="btn btn-secondary"><Icon name="bookmark" /> Saved view</button>
          <button className="btn btn-primary"><Icon name="download" /> Download CSV</button>
        </>} />
      ...
      <div className="list-layout">
        <FilterRail onClear={() => { setStateFilter([]); setPlanFilter([]); }}>
          ...
          <h4>Plan</h4>
          <div className="filter-group">
            {[{value:'pro',label:'Resume Pro'},{ value:'package',label:'Resume Package'}].map(o => {
              const on = planFilter.includes(o.value);
              return <label key={o.value} className={`filter-chip ${on ? 'is-on' : ''}`} onClick={() => setPlanFilter(s => s.includes(o.value) ? s.filter(x => x !== o.value) : [...s, o.value])}><span className="filter-chip-cb">{on && <Icon name="check" size={9} />}</span><span>{o.label}</span></label>;
            })}
          </div>
          <h4>Writer</h4>
          <div className="filter-group">
        ...
        <div className="table-wrap">
          ...
          <table className="tbl">
            <thead><tr>
              <th>Order</th><th>Candidate</th><th>Plan</th><th>Status</th>
            ...
            <tbody>
              {filtered.map(o => (
                <tr key={o.id} onClick={() => openOrder && openOrder(o.id)} style={{ cursor: 'pointer' }}>
                  <td className="tnum text-muted">{o.id}</td>
                  <td><div className="av-row"><Avatar initials={o.candidate.avatarInitials} /><div><div className="n">{o.candidate.name}</div><div className="e">{o.candidate.email}</div></div></div></td>
                  <td><span style={{ fontSize: 11, fontWeight: 600, color: o.plan.key === 'package' ? 'var(--violet-strong)' : 'var(--primary-800)', background: o.plan.key === 'package' ? 'var(--violet-soft)' : 'var(--primary-50)', padding: '2px 8px', borderRadius: 4 }}>{o.plan.label}</span></td>
...
function MRRDetailScreen({ orderId, goBack }) {
  ...
  const planColor = order.plan.key === 'package' ? { bg: 'var(--violet-soft)', fg: 'var(--violet-strong)' } : { bg: 'var(--primary-50)', fg: 'var(--primary-800)' };
      ...
      <div style={{ display: 'flex', gap: 20 }}>
        ...
        <aside className="cand-rail" style={{ width: 280, flex: '0 0 280px' }}>
          <div className="cand-rail-card">
            ...
            <div className="cand-rail-section">
              ...
              <div style={{ padding: '10px', background: 'var(--bg-alt)', borderRadius: 8 }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <ScoreBadge value={order.originalScore} />
                </div>
            ...
            <div className="cand-rail-section" style={{ borderTop: '1px solid var(--border-1)' }}>
              <div className="text-xs cap mb-2" style={{ color: 'var(--fg-3)' }}>Plan</div>
              <div style={{ fontWeight: 700, fontSize: 14, color: planColor.fg, marginBottom: 8 }}>{order.plan.label}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {order.plan.services.map(s => (
                  <div key={s} className="flex items-center gap-2 text-xs" style={{ color: 'var(--fg-2)' }}>
                    <Icon name="check" size={11} style={{ stroke: 'var(--green-strong)', flexShrink: 0 }} />{s}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="cand-rail-section" style={{ borderTop: '1px solid var(--border-1)' }}>
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="font-semi text-sm tnum">{fmtINR(order.plan.price)}</span>
              </div>
              {order.rrDiscount > 0 && (
                <div className="flex justify-between items-center mb-1">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div>
                ...
                {order.originalResume ? (
                  <div className="flex items-center gap-3" style={{ padding: '12px 14px', background: 'var(--bg-alt)', borderRadius: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div className="font-semi text-sm">{order.originalResume.name}</div>
                      <div className="text-xs text-muted">Uploaded {fmtDateTime(order.originalResume.uploadedAt)} · Score: <span style={{ fontWeight: 600 }}>{order.originalScore}</span></div>
                    </div>
                    <button className="btn btn-ghost btn-sm"><Icon name="download" size={13} /> Download</button>
                  </div>
                ) : (
                  <div style={{ padding: '14px', background: 'var(--bg-alt)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Icon name="clock" size={14} style={{ stroke: 'var(--fg-3)' }} />
                    <span className="text-sm text-muted">Waiting for candidate to upload resume</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {rewrittenUploaded && order.rewrittenResume ? (
                  ...
                  <div style={{ padding: '14px 16px', background: 'var(--bg-alt)', borderRadius: 8, border: '2px dashed var(--border-2)', display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <button className="btn btn-primary btn-sm" onClick={() => setShowUploadModal(true)} disabled={!order.originalResume}>Upload</button>
                  </div>
                )}
              </div>

              {/* Cover letter (Package only) */}
              {order.plan.key === 'package' && (
                <div>
                  <div className="text-xs cap mb-2" style={{ color: 'var(--fg-3)' }}>Cover letter</div>
                  <div style={{ padding: '12px 14px', background: 'var(--bg-alt)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Icon name="info" size={14} style={{ stroke: 'var(--primary)' }} />
                    <span className="text-sm" style={{ color: 'var(--fg-2)' }}>Cover letter is manually created and sent directly by the recruiter. No upload required here.</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <NotesPanel />
          <ActivityLogPanel events={[
            { icon: 'card', tone: 'violet', when: order.placed, text: `Order placed · ${order.plan.label}`, meta: [fmtINR(order.amountPaid), order.rrDiscount > 0 ? `RR discount -₹99 applied` : null, order.coupon ? `Coupon ${order.coupon}` : null].filter(Boolean) },
            ...(order.originalResume ? [{ icon: 'upload', tone: 'blue', when: order.originalResume.uploadedAt, text: 'Candidate uploaded original resume', meta: [`Score: ${order.originalScore}`] }] : []),
