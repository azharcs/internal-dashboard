...
function RCListScreen({ openOrder }) {
  const [stateFilter, setStateFilter] = React.useState([]);
  const [writerFilter, setWriterFilter] = React.useState([]);
  const allOrders = window.RC_ORDERS_FULL;
  let orders = window.ACCESS.isRestricted() ? window.ACCESS.filterOrders(allOrders, 'writer') : allOrders;
  if (stateFilter.length) orders = orders.filter(o => stateFilter.includes(o.state));
  if (writerFilter.length) orders = orders.filter(o => writerFilter.includes(o.writer));

  const writerNames = [...new Set(allOrders.map(o => o.writer).filter(w => w !== '—'))].sort();
  const counts = {};
  window.RC_STATES_ALL.forEach(s => counts[s] = allOrders.filter(o => o.state === s).length);

  return (
    <div className="page">
      <ServicePageHead code="RC" title="Recruiter Connect" sub="Manual delivery · ₹1,499 · 30-min call + post-call report"
        actions={<button className="btn btn-primary"><Icon name="download" /> Download CSV</button>} />

      <div className="kpi-row mb-4" style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
        <KpiTile label="New" value={allOrders.filter(o => o.state === 'New').length} sub="awaiting assignment" tone="amber" />
        <KpiTile label="Scheduled" value={allOrders.filter(o => o.state === 'Scheduled').length} sub="call booked" tone="blue" />
        <KpiTile label="Delivered" value={allOrders.filter(o => o.state === 'Delivered').length} sub="report sent" tone="green" />
        <KpiTile label="Cancelled" value={allOrders.filter(o => o.state === 'Cancelled').length} sub="" />
      </div>

      <div className="list-layout">
        <FilterRail onClear={() => { setStateFilter([]); setWriterFilter([]); }}>
          ...
          {!window.ACCESS.isRestricted() && (<>
            <h4>Writer</h4>
            <div className="filter-group">
              {['Unassigned', ...writerNames].map(o => (
                <label key={o} className="filter-chip">
                  <input type="checkbox" style={{ display:'none' }} checked={writerFilter.includes(o === 'Unassigned' ? '—' : o)} onChange={() => {
                    const v = o === 'Unassigned' ? '—' : o;
                    setWriterFilter(s => s.includes(v) ? s.filter(x => x !== v) : [...s, v]);
                  }} />
                  <span className="filter-chip-cb"></span><span>{o}</span>
                </label>
              ))}
            </div>
          </>)}
        ...
        <div className="table-wrap">
          <div className="table-toolbar">
            <div className="left"><span className="text-xs text-muted">{orders.length} orders</span></div>
          </div>
          <table className="tbl">
            <thead><tr>
              <th>Candidate</th><th>Purchased</th><th>Status</th><th>Writer</th><th>Paid</th>
            </tr></thead>
            <tbody>
              {orders.map(o => (
                <tr key={o.id} onClick={() => openOrder && openOrder(o.id)} style={{ cursor: 'pointer' }}>
                  <td><div className="av-row"><Avatar initials={o.candidate.avatarInitials} /><div><div className="n">{o.candidate.name}</div><div className="e">{o.candidate.email}</div></div></div></td>
                  <td className="tnum text-sm text-muted">{fmtDateTime(o.placed)}</td>
...
function RCDetailScreen({ orderId, goBack }) {
  ...
  const [currentState, setCurrentState] = React.useState(order.state);
  const [assignedWriter, setAssignedWriter] = React.useState(order.writer);
  const [targetRole, setTargetRole] = React.useState(order.targetRole);
  const [expLevel, setExpLevel] = React.useState(order.experienceLevel);
  const [resume, setResume] = React.useState(order.resume);
  const [jobLinks, setJobLinks] = React.useState(order.jobLinks);
  const [availability, setAvailability] = React.useState(order.availability);
  const [callReport, setCallReport] = React.useState(order.callReport);
  const [editingInfo, setEditingInfo] = React.useState(false);
  const [editTargetRole, setEditTargetRole] = React.useState(order.targetRole || '');
  const [editExpLevel, setEditExpLevel] = React.useState(order.experienceLevel || '');
  const [editJobLinks, setEditJobLinks] = React.useState(order.jobLinks || '');
  const [editAvailability, setEditAvailability] = React.useState(order.availability || '');

  const history = [
    { actor: 'System', action: 'order created', when: order.placed },
    ...(order.writer !== '—' ? [{ actor: 'Ops', action: `assigned writer ${order.writer}`, when: relDate(0, 10, 5) }] : []),
  ];

  const rcRequirements = currentState === 'New' ? [
    { label: 'Target role filled', met: !!targetRole },
    { label: 'Experience level filled', met: !!expLevel },
    { label: 'Resume uploaded', met: !!resume },
  ] : currentState === 'Scheduled' ? [
    { label: 'Post-call report uploaded', met: !!callReport },
  ] : null;

  const rcBlockedTransitions = {};
  if (currentState === 'New' && rcRequirements) {
    const unmet = rcRequirements.filter(r => !r.met);
    if (unmet.length > 0) rcBlockedTransitions['Scheduled'] = unmet.map(r => r.label).join(' · ') + ' required';
  }
  if (currentState === 'Scheduled' && !callReport) {
    rcBlockedTransitions['Delivered'] = 'Upload post-call report first';
  }
  ...
  return (
    <div className="page">
      ...
      <div className="page-head">
        <div>
          <div className="flex items-center gap-3"><h1>{order.id}</h1><Pill tone={window.rcStateTone(currentState)} dot>{currentState}</Pill></div>
      ...
      <div style={{ display: 'flex', gap: 20 }}>
        <ContextRail candidate={order.candidate} score={order.score}
          payment={{ amount: order.amountPaid, when: order.placed }} />

        <div style={{ flex: 1, minWidth: 0 }}>
          <StatusTransitionPanel currentState={currentState} transitions={window.RC_TRANSITIONS}
            tone={window.rcStateTone} history={history} onTransition={setCurrentState}
            requirements={rcRequirements} blockedTransitions={rcBlockedTransitions}
            writerNode={!window.ACCESS.isRestricted() && <window.WriterPickerBtn value={assignedWriter} onChange={setAssignedWriter} />} />
          ...
          <div className="card mb-4">
            <div className="card-head">
              ...
              {!editingInfo
                ? <button className="btn btn-secondary btn-sm" onClick={() => setEditingInfo(true)}><Icon name="edit" size={12} /> Edit</button>
                : <div className="flex gap-2">
                    <button className="btn btn-ghost btn-sm" onClick={() => setEditingInfo(false)}>Cancel</button>
                    <button className="btn btn-primary btn-sm" onClick={() => {
                      setTargetRole(editTargetRole || null);
                      setExpLevel(editExpLevel || null);
                      setJobLinks(editJobLinks || null);
                      setAvailability(editAvailability || null);
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <div className="text-xs cap mb-1" style={{ color: 'var(--fg-3)' }}>Target role</div>
                {editingInfo
                  ? <input className="t500-input" value={editTargetRole} onChange={e => setEditTargetRole(e.target.value)} placeholder="e.g. Senior Product Manager" style={{ width: '100%', padding: '8px 10px' }} />
                  : <div className="text-sm" style={{ color: targetRole ? 'var(--fg-1)' : 'var(--fg-4)' }}>{targetRole || 'Not provided'}</div>
                }
              </div>
              <div>
                <div className="text-xs cap mb-1" style={{ color: 'var(--fg-3)' }}>Experience level</div>
                {editingInfo
                  ? <select className="t500-input" value={editExpLevel} onChange={e => setEditExpLevel(e.target.value)} style={{ width: '100%', padding: '8px 10px' }}>
                      <option value="">Select level</option>
                      {['Mid-level (3–5 yrs)','Senior (6–9 yrs)','Lead / Principal (10+ yrs)'].map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                  : <div className="text-sm" style={{ color: expLevel ? 'var(--fg-1)' : 'var(--fg-4)' }}>{expLevel || 'Not provided'}</div>
                }
              </div>
              <div>
                <div className="text-xs cap mb-1" style={{ color: 'var(--fg-3)' }}>Resume</div>
                {resume
                  ? <div className="flex items-center gap-2" style={{ padding: '8px 10px', background: 'var(--bg-alt)', borderRadius: 6 }}>
                      <Icon name="file" size={14} style={{ stroke: 'var(--fg-3)' }} />
                      <span className="text-sm font-semi" style={{ flex: 1 }}>{resume.name}</span>
                      <span className="text-xs text-muted">Uploaded {fmtDateTime(resume.uploadedAt)}</span>
                      <label className="btn btn-ghost btn-sm" style={{ cursor: 'pointer' }}>Replace<input type="file" accept=".pdf" style={{ display:'none' }} onChange={e => { const f=e.target.files[0]; if(f) setResume({name:f.name,uploadedAt:new Date()}); }} /></label>
                    </div>
                  : <div style={{ border: '1.5px dashed var(--border-2)', borderRadius: 6, padding: '12px 14px', textAlign: 'center' }}>
                      <div className="text-sm text-muted mb-2">No resume uploaded</div>
                      <label className="btn btn-secondary btn-sm" style={{ cursor: 'pointer' }}><Icon name="upload" size={12} /> Upload PDF<input type="file" accept=".pdf" style={{ display:'none' }} onChange={e => { const f=e.target.files[0]; if(f) setResume({name:f.name,uploadedAt:new Date()}); }} /></label>
                    </div>
                }
              </div>
              <div>
                <div className="text-xs cap mb-1" style={{ color: 'var(--fg-3)' }}>Job links</div>
                {editingInfo
                  ? <textarea className="t500-input" value={editJobLinks} onChange={e => setEditJobLinks(e.target.value)} placeholder="One link per line…" rows={3} style={{ width: '100%', padding: '8px 10px', resize: 'vertical' }} />
                  : jobLinks
                    ? <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {jobLinks.split('\n').map((l,i) => l.trim() && <a key={i} href={l.trim()} target="_blank" rel="noopener noreferrer" className="text-sm" style={{ color:'var(--primary)', wordBreak:'break-all' }}>{l.trim()}</a>)}
                      </div>
                    : <div className="text-sm" style={{ color: 'var(--fg-4)' }}>Not provided</div>
                }
              </div>
              <div>
                <div className="text-xs cap mb-1" style={{ color: 'var(--fg-3)' }}>Availability</div>
                {editingInfo
                  ? <input className="t500-input" value={editAvailability} onChange={e => setEditAvailability(e.target.value)} placeholder="e.g. Weekdays 6–8 PM IST" style={{ width: '100%', padding: '8px 10px' }} />
                  : <div className="text-sm" style={{ color: availability ? 'var(--fg-1)' : 'var(--fg-4)' }}>{availability || 'Not provided'}</div>
                }
              </div>
            </div>
          </div>

          {/* Post-call report card */}
          {['Scheduled','Delivered'].includes(currentState) && (
            <div className="card mb-4">
              <div className="card-head">
                <h3 className="card-title">Post-call report</h3>
                {!callReport && currentState === 'Scheduled' && (
                  <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--amber-strong)', background: 'var(--amber-soft)', padding: '2px 8px', borderRadius: 4 }}>Required to deliver</span>
                )}
              </div>
              <div style={{ padding: 20 }}>
                {callReport ? (
                  <div className="flex items-center gap-3" style={{ padding: '12px 14px', background: 'var(--bg-alt)', borderRadius: 8 }}>
                    <Icon name="file" size={18} style={{ stroke: 'var(--green-strong)' }} />
                    <div style={{ flex: 1 }}>
                      <div className="font-semi text-sm">{callReport.name}</div>
                      <div className="text-xs text-muted">Uploaded {fmtDateTime(callReport.uploadedAt)}</div>
                    </div>
                    <Pill tone="green" dot>Uploaded</Pill>
                    <label className="btn btn-ghost btn-sm" style={{ cursor: 'pointer' }}>Replace<input type="file" accept=".pdf" style={{ display:'none' }} onChange={e => { const f=e.target.files[0]; if(f) setCallReport({name:f.name,uploadedAt:new Date()}); }} /></label>
                  </div>
                ) : (
                  <div style={{ border: '1.5px dashed var(--border-2)', borderRadius: 8, padding: '24px 16px', textAlign: 'center' }}>
                    <Icon name="upload" size={24} style={{ stroke: 'var(--fg-3)', marginBottom: 8, display: 'block', margin: '0 auto 8px' }} />
                    <div className="text-sm font-semi mb-1">Upload post-call report</div>
                    <div className="text-xs text-muted mb-3">PDF only · Max 10 MB</div>
                    <label className="btn btn-primary btn-sm" style={{ cursor: 'pointer' }}><Icon name="upload" size={12} /> Choose PDF<input type="file" accept=".pdf" style={{ display:'none' }} onChange={e => { const f=e.target.files[0]; if(f) setCallReport({name:f.name,uploadedAt:new Date()}); }} /></label>
                  </div>
                )}
              </div>
            </div>
          ...
          <NotesPanel />
          <ActivityLogPanel events={[
            { icon: 'card', tone: 'violet', when: order.placed, text: 'Order placed', meta: [fmtINR(order.amountPaid)].filter(Boolean) },
            ...(order.writer !== '—' ? [{ icon: 'user', tone: 'green', when: relDate(0, 10, 0), text: `Writer assigned: ${order.writer}`, meta: [] }] : []),
            ...(order.callReport ? [{ icon: 'file', tone: 'blue', when: order.callReport.uploadedAt, text: 'Post-call report uploaded', meta: [] }] : []),
